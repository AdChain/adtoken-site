...
const Discovery = () => {
  useEffect(() => {
    ...
    const loadAnimation = async () => {
      ...
      const jsonfile = "/rover1.json"; // Adjust the path to your JSON file

      // Make AJAX request to fetch JSON data
      const response = await fetch(jsonfile);
      const json = await response.json();
      // Initialize animation
      const container: any = document.getElementById("discovery-animation");
      // @ts-ignore
  ...
  return (
    <>
      <div id="discovery-animation"></div>
    </>
  );
};
